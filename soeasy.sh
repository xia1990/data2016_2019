...
function signapk() {
    ...
    sign_key1=device/mediatek/common/security/E286L/platform.x509.pem
    sign_key2=device/mediatek/common/security/E286L/platform.pk8
    echo "please wait..."
	for file in $@; do
        ...
        cp ${apk_name}_signed.apk /data/mine/test/$uid_name/
...
function signzip() {
    uid_name=`whoami`
    sign_tool=out/host/linux-x86/framework/signapk.jar
    sign_key1=device/mediatek/common/security/D281L/releasekey.x509.pem
    sign_key2=device/mediatek/common/security/D281L/releasekey.pk8
    echo "please wait..."
	for file in $@; do
        file_name=$(echo $file | sed -r 's/\.zip$//i')
        java -Djava.library.path=out/host/linux-x86/lib64 -jar $sign_tool -w $sign_key1 $sign_key2 $file ${file_name}_signed.zip
